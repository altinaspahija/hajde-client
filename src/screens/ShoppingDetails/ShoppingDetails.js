...
export default function ShoppingDetails({ navigation, route }) {
  ...
  const anuloPorosin = async () => {
    try {
      ...
      navigation.navigate("Porositë");
    } catch (err) {
      Alert.alert("Mesazhi", err?.message, [{ text: "Në rregull" }]);
    }
  };
        ...
        <View style={styles.indicatorContainer}>
          ...
          <View style={styles.ordersContainer}>
            {products && products?.length != 0 ? (
              ...
              <Text>Ndodhi një problem kontakto administratorin</Text>
            )}

            {manualData && manualData?.length != 0 && products?.length === 0 ? (
